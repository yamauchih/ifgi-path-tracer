* Task ifgi path tracer 2011-8-5(Fri)
...
** TODO view all to see the cornel box: change the z_near and z_far

Examner widget self.__scene_cog と self.__scene_radius を set している部分を探す
それを以下の関数でおきかえる

void QtExaminerWidget::setScenePos(const base::Vec3f& _cog, float _radius) {
    d_cog = _cog;
    d_radius = _radius;

    d_camera.lock();
    d_camera.lookAt(_cog);
    d_camera.orthoWidth(2.0*d_radius);
    ここで clipping plane が変化する
    d_camera.setClippingPanes(0.01*_radius, 500.0*_radius);
    d_camera.focalLength(_radius);
    d_camera.unlock();
    updateGL();
}

void QtExaminerWidget::setSceneSize(float _radius) {
    d_radius = _radius;
    d_camera.lock();
    d_camera.orthoWidth(2.0*d_radius);
    ここで clipping plane が変化する
    d_camera.setClippingPanes(0.01*_radius, 10.0*_radius);
    d_camera.focalLength(_radius);
    d_camera.unlock();
}
